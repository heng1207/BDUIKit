...
- (void)lc_verticalAlignmentWithTitleTop:(BOOL)isTop space:(float)space ;
{
    ...
    if (isTop) {
        ...
        [self setContentEdgeInsets:UIEdgeInsetsMake(halfHeight+space, leftInset, -bottomInset, -rightInset)];
    } else {
        [self setTitleEdgeInsets:UIEdgeInsetsMake(halfHeight+space, - halfWidth, -halfHeight-space, halfWidth)];
        [self setContentEdgeInsets:UIEdgeInsetsMake(-bottomInset, leftInset, halfHeight+space, -rightInset)];
    }
}
